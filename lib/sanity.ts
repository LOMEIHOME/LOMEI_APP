...
function adaptProyecto(p: SanityProyecto): Proyecto {
  return {
    ...
    area: p.area || "",
    duration: p.duration || "",
    images: p.images?.filter((img) => img.asset).map((img) => urlFor(img).width(1200).quality(80).format("webp").url()) || [],
    featured: p.featured || false,
  };
}

function adaptProducto(p: SanityProducto): Producto {
  const images = p.images?.filter((img) => img.asset).map((img) => urlFor(img).width(800).quality(80).format("webp").url()) || [];
  return {
    slug: p.slug.current,
    name: p.name,
