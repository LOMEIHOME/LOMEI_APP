...
interface Props {
  proyectos: FeaturedProject[];
}

export default function ProjectsGrid({ proyectos }: Props) {
  const [active, setActive] = useState(0);
  const [paused, setPaused] = useState(false);

  const next = useCallback(() => {
    setActive((i) => (i + 1) % proyectos.length);
  }, [proyectos.length]);

  const prev = useCallback(() => {
    setActive((i) => (i - 1 + proyectos.length) % proyectos.length);
  }, [proyectos.length]);

  // Auto-advance every 3s
  useEffect(() => {
    if (paused || proyectos.length === 0) return;
    const timer = setInterval(next, 3000);
    return () => clearInterval(timer);
  }, [paused, next, proyectos.length]);
